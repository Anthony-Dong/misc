...
public class ServerApplicationListener implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        IntStream.range(0, size).forEach((e) -> {
            ...
            // 创建一个客户端
            ChatServer chatServer = new ChatServer(socketAddress, redisPool, counter, new ChatBootListener() {
                @Override
                public void onChatBootEvent(ChatBootEvent event) throws InterruptedException, IOException, KeeperException {

                    ChatBootSource source = (ChatBootSource) event.getSource();
                    ...
                    if (source.isSuccess()) {
                        ...
                        // key值   /chat/唯一ID
                        String key = configs.getZookeeperPath() + "/" + SNOWFLAKE.nextId();

                        // 每一个节点的信息其实就是查询这个节点 的 地址信息
                        String value = pre + "?ip=" + nettyIp;

                        // 启动zookeeper 客户端  , 这个是创建了一个临时节点 ,当这个线程挂掉的时候 zk就挂了
                        zooKeeper = bootZookeeper(key, value);

                    } else if (source.isFailed()) {
                        // 失败操作 ...

                        // 把这个移除掉 ....
                        countMap.remove(nettyIp);
                        ...
                        if (null != source.hasOtherMsg()) {
                            log.info("[服务器-{}] 与 [客户端-{}] 断开连接", socketAddress, source.hasOtherMsg());
                        }
