...
public class RpcProxy<T> implements InvocationHandler {
    ...
    public static <T> T newInstance(Class<T> inter, RpcContext context) throws ProxyException {
        return newInstance(inter, context, Constants.DEFAULT_TIMEOUT);
    }


    @SuppressWarnings("all")
    public static <T> T newInstance(Class<T> inter, RpcContext context, long timeout) throws ProxyException {
        ...
        RpcProxy<T> rpcProxy = new RpcProxy<>(context, inter, timeout);
        try {
